...
@task()
def query_outdated(ctx, begin="2019-12-05", end="2020-01-05"):
    begin_period = arrow.get(begin)
    end_period = arrow.get(end)
    datasets = requests.get("https://transport.data.gouv.fr/api/datasets").json()
    print("dataset,aom,modified date,end validity")
    ...
    for d in datasets:
        ...
        sorted_history = sorted(
            history, key=lambda h: arrow.get(h["last_modified"]), reverse=True
        )

        for h in sorted_history[1:]:
            last_validity_period = h["metadata"].get("end")

            end_date = arrow.get(last_validity_period)
            if end_date is None:
                continue

            if end_date > begin_period and end_date < end_period:
                modified = arrow.get(h["last_modified"])
                if modified > begin_period and modified < end_period:
                    print(
                        f'{d["title"]},{d["aom"].get("name")},{arrow.get(h["last_modified"])},{last_validity_period}'
                    )
